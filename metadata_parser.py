...
def get_name_suggestion(text_list: list) -> str:
    ...
    # Clean the text by removing extra spaces, double spaces, newlines, etc.
    cleaned_text = re.sub(r"\s+", " ", text_str).strip()
    logger.debug(f"Cleaned text:\n{cleaned_text}")
    dates = extract_dates(cleaned_text)

    # Return the most recent that is not in the future
